...
run "default_values" {
  ...
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.sku_name == "S0"
    error_message = "sku_name must default to S0"
  }
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.kind == "OpenAI"
    error_message = "kind must default to OpenAI"
  }
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.location == "canadacentral"
...
run "identity_system_assigned" {
  ...
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.identity[0].type == "SystemAssigned"
...
run "identity_user_assigned" {
  ...
  assert {
    condition     = length(azurerm_cognitive_account.cognitive-account.identity[0].identity_ids) == 1
...
run "no_identity" {
  ...
  assert {
    condition     = length(azurerm_cognitive_account.cognitive-account.identity) == 0
...
run "customer_managed_key" {
  ...
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.customer_managed_key[0].key_vault_key_id == "https://my-kv.vault.azure.net/keys/cmk/00000000000000000000000000000000"
...
run "network_acls_ip_rules" {
  ...
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.network_acls[0].default_action == "Deny"
    error_message = "network_acls.default_action must be passed through"
  }
  assert {
    condition     = tolist(azurerm_cognitive_account.cognitive-account.network_acls[0].ip_rules)[0] == "203.0.113.10"
...
run "network_acls_bypass" {
  ...
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.network_acls[0].bypass == "AzureServices"
...
run "network_acls_virtual_network_rules" {
  ...
  assert {
    condition     = tolist(azurerm_cognitive_account.cognitive-account.network_acls[0].virtual_network_rules)[0].subnet_id == "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg-net/providers/Microsoft.Network/virtualNetworks/vnet-main/subnets/snet-ai"
    error_message = "virtual_network_rules.subnet_id must be passed through"
  }
  assert {
    condition     = tolist(azurerm_cognitive_account.cognitive-account.network_acls[0].virtual_network_rules)[0].ignore_missing_vnet_service_endpoint == true
...
run "storage_accounts" {
  ...
  assert {
    condition     = length(azurerm_cognitive_account.cognitive-account.storage) == 1
...
run "network_injection" {
  ...
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.network_injection[0].scenario == "agent"
...
run "no_network_injection" {
  ...
  assert {
    condition     = length(azurerm_cognitive_account.cognitive-account.network_injection) == 0
...
run "project_management_enabled" {
  ...
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.project_management_enabled == true
...
run "qna_runtime_endpoint" {
  ...
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.qna_runtime_endpoint == "https://my-qna-runtime.azurewebsites.net"
...
run "custom_question_answering" {
  ...
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.custom_question_answering_search_service_id == "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg-search/providers/Microsoft.Search/searchServices/search-svc"
    error_message = "custom_question_answering_search_service_id must be passed through"
  }
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.custom_question_answering_search_service_key == "search-service-admin-key"
...
run "metrics_advisor_arguments" {
  ...
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.metrics_advisor_aad_client_id == "33333333-3333-3333-3333-333333333333"
    error_message = "metrics_advisor_aad_client_id must be passed through"
  }
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.metrics_advisor_aad_tenant_id == "44444444-4444-4444-4444-444444444444"
    error_message = "metrics_advisor_aad_tenant_id must be passed through"
  }
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.metrics_advisor_super_user_name == "admin@example.org"
    error_message = "metrics_advisor_super_user_name must be passed through"
  }
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.metrics_advisor_website_name == "my-metrics-advisor-site"
...
run "custom_tags_merge" {
  ...
  assert {
    condition     = azurerm_cognitive_account.cognitive-account.tags["Owner"] == "team-a"
